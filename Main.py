...
def error(error_msg = 'Bozulan/goldawy bolmadyk fayl.'):
...
def main():
    ...
    parser.add_argument('file', help='Döwmek üçin faýl')
    ...
    if file_ext not in PASSWORDS:
        warn(f'Näbelli fayl giňeltmesi, {DEFAULT_FILE_EXTENSION} giňeltmesine geçilýär')
    ...
    if args.output:
        ...
        if output_file_path.exists() and output_file_path.is_file():
            ...
            if not human_bool_to_bool(ask(f'"{args.output}" Fayl eýýäm bar. Üstüne ýazylsynmy? (y/n)')):
...
if __name__ == '__main__':
    print('\n\n\nSkripty ýasan 𝑨𝒎𝒂𝒏𝒐𝒇𝒇 𝑶𝒇𝒇𝒊𝒄𝒊𝒂𝒍\n* t.me/Official_amanoff\n* t.me/hzm_hacker\n==========================') 
    args = sys.argv    
    file = args[1]
    if file.split('.')[1] == 'ssh':
            sshinjector(file)
